...
void criar_titulo_idx() {
    ...
    for (int i = 0; i < qtd_registros_cursos; i++) {
        Curso c = recuperar_registro_curso(i);
        char aux[TAM_MAX_TITULO]; // aux é utilizado para deixar o título em letra maiúscula.

        strcpy(aux, c.titulo);
        strupr(aux);

        strcpy(titulo_idx[i].id_curso, c.id_curso);
        strcpy(titulo_idx[i].titulo, aux);
    }
...
void criar_data_curso_usuario_idx() {
    ...
    qsort(data_curso_usuario_idx, qtd_registros_inscricoes, sizeof(data_curso_usuario_index), qsort_data_idx);
...
void criar_categorias_idx() {
    if (!categorias_idx.categorias_primario_idx)
        categorias_idx.categorias_primario_idx = malloc(3 * MAX_REGISTROS * sizeof(categorias_primario_index));
    ...
    if (!categorias_idx.categorias_secundario_idx)
        categorias_idx.categorias_secundario_idx = malloc(3 * MAX_REGISTROS * sizeof(categorias_secundario_index));
...
void cadastrar_usuario_menu(char *id_usuario, char *nome, char *email, char *telefone) {
    usuarios_index *p;
    p = (usuarios_index *) busca_binaria(id_usuario, usuarios_idx, qtd_registros_usuarios, sizeof(usuarios_index), qsort_usuarios_idx, false, 0);
        
    if (p != NULL && p->rrn != -1) // Caso em que um usuário com o id informado já existe.
...
void cadastrar_telefone_menu(char* id_usuario, char* telefone) {
    ...
    if (p != NULL && !strcmp(p->id_usuario, id_usuario) && p->rrn != -1) {
        // Posiciona iArq no local correto para atualizar o telefone.
        for (iArq = p->rrn * TAM_REGISTRO_USUARIO; cont < 3; iArq++) {
...
void remover_usuario_menu(char *id_usuario) {
    ...
    if (p != NULL && !strcmp(p->id_usuario, id_usuario) && p->rrn != -1) {
        ARQUIVO_USUARIOS[p->rrn * TAM_REGISTRO_USUARIO] = '*';
        ARQUIVO_USUARIOS[p->rrn * TAM_REGISTRO_USUARIO + 1] = '|';            
        p->rrn = -1;
...
void cadastrar_curso_menu(char *titulo, char *instituicao, char *ministrante, char* lancamento, int carga, double valor) {
    ...
    p = (titulos_index *) busca_binaria(aux, titulo_idx, qtd_registros_cursos, sizeof(titulos_index), qsort_titulo_idx , false, 0);
    ...
    if (p == NULL) {
        ...
        strcpy(titulo_idx[qtd_registros_cursos].titulo, aux);
...
void adicionar_saldo_menu(char *id_usuario, double valor) {
    double saldoAtual;
    int cont = 0, iArq;
    char p[TAM_FLOAT_NUMBER];
    p[0] = '\0';
    usuarios_index *pont;

    pont = (usuarios_index *) busca_binaria(id_usuario, usuarios_idx, qtd_registros_usuarios, sizeof(usuarios_index), qsort_usuarios_idx, false, 0);
    
    if (pont != NULL && pont->rrn != -1) {
        if (valor <= 0) {
            printf(ERRO_VALOR_INVALIDO);
        }
        else {    
            // Coloca iArq na posição correta.
            for (iArq = pont->rrn * TAM_REGISTRO_USUARIO; cont < 4; iArq++) {
                if (ARQUIVO_USUARIOS[iArq] == ';')
                    cont += 1;
            }
...
void inscrever_menu(char *id_curso, char *id_usuario) {
    cursos_index *cIdx;
    usuarios_index *uIdx;
    inscricoes_index *iIdx = malloc(sizeof(inscricoes_index));

    cIdx = (cursos_index *) busca_binaria(id_curso, cursos_idx, qtd_registros_cursos, sizeof(cursos_index), qsort_cursos_idx, false, 0);
    uIdx = (usuarios_index *) busca_binaria(id_usuario, usuarios_idx, qtd_registros_usuarios, sizeof(usuarios_index), qsort_usuarios_idx, false, 0);

    if (uIdx == NULL || cIdx == NULL) // Usuário e/ou curso não existem.
        printf(ERRO_REGISTRO_NAO_ENCONTRADO);
    else {
        strcpy(iIdx->id_curso, id_curso);
        strcpy(iIdx->id_usuario, id_usuario);
        iIdx = (inscricoes_index *) busca_binaria(iIdx, inscricoes_idx, qtd_registros_inscricoes, sizeof(inscricoes_index), qsort_inscricoes_idx, false, 0);

        if (iIdx == NULL) {
            Inscricao ins;
            Usuario u = recuperar_registro_usuario(uIdx->rrn);
            Curso c = recuperar_registro_curso(cIdx->rrn);
            char buffer[14]; // Utilizado para obter a data atual.
            ...
            if (u.saldo < c.valor)
                ...
                strcpy(data_curso_usuario_idx[qtd_registros_inscricoes].data, ins.data_inscricao);
...
void cadastrar_categoria_menu(char* titulo, char* categoria) {
    ...
    t = (titulos_index *) busca_binaria(aux, titulo_idx, qtd_registros_cursos, sizeof(titulos_index), qsort_titulo_idx , false, 0);

    if (t == NULL)
        printf(ERRO_REGISTRO_NAO_ENCONTRADO);
    else {
        p = (cursos_index *) busca_binaria(t->id_curso, cursos_idx, qtd_registros_cursos, sizeof(cursos_index), qsort_cursos_idx , false, 0);
        inverted_list_secondary_search(&result, false, categoria, &categorias_idx);

        // Verifrica se o curso já possui a categoria fornecida.
        while (result != -1) {
            if (!strcmp(categorias_idx.categorias_primario_idx[result].chave_primaria, p->id_curso)) {
                printf(ERRO_CATEGORIA_REPETIDA, titulo, categoria);
                break;
            }
            
            result = categorias_idx.categorias_primario_idx[result].proximo_indice;
        }
        
        if (result == -1) { // Curso não possui a categoria.
            Curso c = recuperar_registro_curso(p->rrn);

            // Atualiza o registro com a nova categoria.
            for (int i = 0; i < 3; i++) {
                if (c.categorias[i][0] == '\0') {
                    strcpy(c.categorias[i], categoria);
                    escrever_registro_curso(c, p->rrn);
                    break;
                }
            }

            inverted_list_insert(categoria, p->id_curso, &categorias_idx); // Atualiza o índice categorias_idx.
...
void atualizar_status_inscricoes_menu(char *id_usuario, char *titulo, char status) {
    titulos_index *t;
    inscricoes_index *iIdx = malloc(sizeof(inscricoes_index));
    char aux[TAM_MAX_TITULO], buffer[14]; // aux é utilizado para deixar o título em letra maiúscula e buffer para obter a data atual.
    ...
    if (t != NULL) {
        strcpy(iIdx->id_usuario, id_usuario);
        strcpy(iIdx->id_curso, t->id_curso);
        iIdx = (inscricoes_index *) busca_binaria(iIdx, inscricoes_idx, qtd_registros_inscricoes, sizeof(inscricoes_index), qsort_inscricoes_idx, false, 0);
        ...
        if (iIdx == NULL)
            ...
            ins = recuperar_registro_inscricao(iIdx->rrn);
            ins.status = status;
            current_datetime(buffer);
            strncpy(ins.data_atualizacao, buffer, 13);
            escrever_registro_inscricao(ins, iIdx->rrn);
...
void buscar_usuario_id_menu(char *id_usuario) {
    ...
    p = busca_binaria(id_usuario, usuarios_idx, qtd_registros_usuarios, sizeof(usuarios_index), qsort_usuarios_idx, true, 0);

    if (p == NULL)
        printf(ERRO_REGISTRO_NAO_ENCONTRADO);
    else
        exibir_usuario(p->rrn);
}
...
void buscar_curso_id_menu(char *id_curso) {
    ...
    p = busca_binaria(id_curso, cursos_idx, qtd_registros_cursos, sizeof(cursos_index), qsort_cursos_idx, true, 0);
...
void buscar_curso_titulo_menu(char *titulo) {
    ...
    t = busca_binaria(aux, titulo_idx, qtd_registros_cursos, sizeof(titulos_index), qsort_titulo_idx, true, 0);

    if (t == NULL)
        printf(ERRO_REGISTRO_NAO_ENCONTRADO);
    else {
        p = busca_binaria(t->id_curso, cursos_idx, qtd_registros_cursos, sizeof(cursos_index), qsort_cursos_idx, true, 0);
...
void listar_cursos_categorias_menu(char *categoria) {
    ...
    if (qtd_registros_cursos != 0 && inverted_list_secondary_search(&resultSecondary, false, aux, &categorias_idx)) {
        qtd_chaves = inverted_list_primary_search(resultPrimary, true, resultSecondary, NULL, &categorias_idx);

        cursos_index *cursosEncontrados = malloc(qtd_chaves * sizeof(cursos_index));
        cursos_index *p;

        for (int i = 0; i < qtd_chaves; i++) {
            p = (cursos_index *) busca_binaria(resultPrimary[i], cursos_idx, qtd_registros_cursos, sizeof(cursos_index), qsort_cursos_idx, false, 0);

            strcpy(cursosEncontrados[i].id_curso, p->id_curso);
            cursosEncontrados[i].rrn = p->rrn;
        }

        qsort(cursosEncontrados, qtd_chaves, sizeof(cursos_index), qsort_cursos_idx);

        for (int i = 0; i < qtd_chaves; i++)
            exibir_curso(cursosEncontrados[i].rrn);

        free(cursosEncontrados);
    }
    else
        printf(AVISO_NENHUM_REGISTRO_ENCONTRADO);
}
...
void listar_inscricoes_periodo_menu(char *data_inicio, char *data_fim) {  
    if (qtd_registros_inscricoes == 0)
        ...
        data_curso_usuario_index *pIni = malloc(sizeof(data_curso_usuario_index));
        data_curso_usuario_index *pFim = malloc(sizeof(data_curso_usuario_index));
        int iData;

        // Busca pela data_inicio ou a imediatamente posterior e pela data_fim ou a imediatamente anterior. 
        pIni = (data_curso_usuario_index *) busca_binaria(data_inicio, data_curso_usuario_idx, qtd_registros_cursos, sizeof(data_curso_usuario_index), qsort_data_idx, 1, 1);
        pFim = (data_curso_usuario_index *) busca_binaria(data_fim, data_curso_usuario_idx, qtd_registros_cursos, sizeof(data_curso_usuario_index), qsort_data_idx, 0, -1); 

        if (strcmp(pIni->data, data_fim) <= 0 || strcmp(pFim->data, data_inicio) >= 0) { // Verifica se as datas obtidas estão fora do intervalo.
            for (iData = 0; iData < qtd_registros_cursos; iData++) {
                if (!strcmp(data_curso_usuario_idx[iData].data, pIni->data))
                    break;
            }

            // Imprime as datas em ordem cronológica.
            while (true) {
                inscricoes_index *ins = malloc(sizeof(inscricoes_index));
                strcpy(ins->id_usuario, data_curso_usuario_idx[iData].id_usuario);
                strcpy(ins->id_curso, data_curso_usuario_idx[iData].id_curso);
                ins = (inscricoes_index *) busca_binaria(ins, inscricoes_idx, qtd_registros_inscricoes, sizeof(inscricoes_index), qsort_inscricoes_idx, false, 0);
                
                if (ins)
                    exibir_inscricao(ins->rrn);
                
                if (!strcmp(data_curso_usuario_idx[iData].data, pFim->data))
                    break;
...
/* Liberar espaço */
void liberar_espaco_menu() {
    char ARQUIVO_AUX[TAM_ARQUIVO_USUARIOS], aux[TAM_REGISTRO_USUARIO + 1];
    ...
    // Atualiza o índice.
    for (int i = 0; i < qtd_usuarios_nao_removidos; i++) {
        usuarios_idx[i].rrn = i;
        strncpy(usuarios_idx[i].id_usuario, ARQUIVO_USUARIOS + (i * TAM_REGISTRO_USUARIO), TAM_ID_USUARIO - 1);
    }
...
int qsort_inscricoes_idx(const void *a, const void *b) {
    if (!strcmp( ( (inscricoes_index *)a )->id_curso, ( (inscricoes_index *)b )->id_curso))
...
int qsort_data_curso_usuario_idx(const void *a, const void *b) {
    if (!strcmp( ( (data_curso_usuario_index *)a )->id_curso, ( (data_curso_usuario_index *)b )->id_curso))
        return strcmp( ( (data_curso_usuario_index *)a )->id_usuario, ( (data_curso_usuario_index *)b )->id_usuario);
    else
        return strcmp( ( (data_curso_usuario_index *)a )->id_curso, ( (data_curso_usuario_index *)b )->id_curso);
}
...
void inverted_list_insert(char *chave_secundaria, char *chave_primaria, inverted_list *t) {
    ...
    if (inverted_list_secondary_search(&result, false, aux, t)) {
        while (true) {
            if (t->categorias_primario_idx[result].proximo_indice == -1)
                break;
            else
                result = t->categorias_primario_idx[result].proximo_indice;                    
        }

        t->categorias_primario_idx[result].proximo_indice = t->qtd_registros_primario;
    }
    else {
        strcpy(t->categorias_secundario_idx[t->qtd_registros_secundario].chave_secundaria, aux);
...
bool inverted_list_secondary_search(int *result, bool exibir_caminho, char *chave_secundaria, inverted_list *t) {
    categorias_secundario_index *p;
    
    p = (categorias_secundario_index *) busca_binaria(chave_secundaria, t->categorias_secundario_idx, t->qtd_registros_secundario, sizeof(categorias_secundario_index), qsort_categorias_secundario_idx, exibir_caminho, 0);

    if (p == NULL) { // Chave não foi encontrada.
        *result = -1;
        return false;
    }
    else { // Chave foi encontrada.
...
int inverted_list_primary_search(char result[][TAM_CHAVE_CATEGORIAS_PRIMARIO_IDX], bool exibir_caminho, int indice, int *indice_final, inverted_list *t) {
    ...
    strcpy(result[qtd_chaves], t->categorias_primario_idx[indice].chave_primaria);
    qtd_chaves += 1;
    
    // Percorre os elementos pertencentes a mesma categoria que o índice e armazena a chave_primaria deles em result.
...
void* busca_binaria(const void *key, const void *base0, size_t nmemb, size_t size, int (*compar)(const void *, const void *), bool exibir_caminho, int retorno_se_nao_encontrado) {
    int menor = 0, maior = nmemb - 1, meio = 0;
    ...
    while (menor <= maior) {
        ...
        if (compar(&base0[meio * size], key) == 0) {
            if (exibir_caminho == true)
                printf("\n");
            
            return (void*) &base0[meio * size];
        }
        else if (compar(&base0[meio * size], key) > 0)
            maior = meio - 1;
        else
            menor = meio + 1;
    }
